...
public class RandomLinkedMinPQ<Key extends Comparable<? super Key>>{
  ...
  public RandomLinkedMinPQ(){}
   
  public RandomLinkedMinPQ(Key[] z){
  ...
  private void swim(Node x){
    ...
    if (greater(x.parent, x)) { exch(x, x.parent); swim(x.parent); }
  }
  ...
  private void sink(Node x){
    ...
    if(x.left == null && x.right == null) return;
      ...
      node = x.left;
      if (greater(x.left, x.right)) node = x.right;
      if (!greater(x, node)) return;
      exch(node, x); sink(node);
    }
  }
...
//  }
  ...
  public boolean isMinHeap(Node x) {
    if (x == null || size(x) == 1) return true;
    else if (size(x) == 2) {
      if (x.right != null) return false;
      if (greater(x.data, x.left.data)) return false;
      return isMinHeap(x.left);
    }
    else {
      if (size(x) < 3 || x.left == null || x.right == null) return false;
      if (greater(x.data,x.left.data) || greater(x.data, x.right.data)) return false;
      return isMinHeap(x.left) && isMinHeap(x.right);
    }
  }
  ...
  public static void main(String[] args) {
    ...
    RandomLinkedMinPQ<Integer> pq; Integer[] a;
    Random r = new Random(System.currentTimeMillis());
    a = rangeInteger(1,21);
    shuffle(a,r);
//    pq = new RandomLinkedMinPQ<>(a);
