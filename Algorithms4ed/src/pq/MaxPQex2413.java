...
public class MaxPQex2413<Key> implements Iterable<Key> {
   ...
   */
  public MaxPQex2413(int initCapacity) {
   ...
   */
  public MaxPQex2413(int initCapacity, Comparator<Key> comparator) {
   ...
   * @param  comparator the order in which to compare the keys
   */
  public MaxPQex2413(Comparator<Key> comparator) {
    this(1, comparator);
  }
  ...
  // based on https://raw.githubusercontent.com/AbhijithMadhav/Data-Structures-And-Algorithms/master/Data%20Structures%20and%20Algorithms/src/ds/pq/MaxPQ.java
  private void sink(int k) {
    Key t = pq[k];
    while (2*k+1 <= n) { // while pq[k] has 2 children
      int j = 2*k;
      if (less(j, j+1)) j++;
      if (!less(k, j)) break;
      exch(k, j);
    ...
    if (2*k == n) // if pq[k] has only one child
      if (less(k, n)) {
        exch(k, n);
        k = n;
      }
  ...
  private class HeapIterator implements Iterator<Key> {
    ...
    // create a new pq
    private MaxPQex2413<Key> copy;

    // add all items to copy of heap
    // takes linear time since already in heap order so no keys move
    public HeapIterator() {
      if (comparator == null) copy = new MaxPQex2413<Key>(size());
      else                    copy = new MaxPQex2413<Key>(size(), comparator);
      for (int i = 1; i <= n; i++)
        copy.insert(pq[i]);
    }
  ...
  public static void printHeaps(String[] s) {
    ...
    for (int i = 0; i < hp.length; i++) {
      MaxPQex2413<String> pq = new MaxPQex2413<String>(hq[i]);
      pq.printHeap();
    }     
  }
  ...
  public static void main(String[] args) {
    ...
    ix = new Integer[]{2,3,4,6,7,2,1,0};
    //    for (int i = 0; i < ix.length; i++) ix[i] = r.nextInt(8);
    par(ix);
    MaxPQex2413<Integer> pq = new MaxPQex2413<Integer>(ix);
