...
public class MaxPQ<Key> implements Iterable<Key> {
  ...
  public MaxPQ(Iterable<Key> itbl) {
    if (itbl == null) {
      pq = (Key[]) new Object[1]; n = 0; return;
    }
    Key[] keys = v.ArrayUtils.toArray(itbl.iterator());
    if (keys.length == 0) {pq = (Key[]) new Object[1]; n = 0; return; }
    n = keys.length; int i = 0; Class<?> kclass;
    pq = (Key[]) new Object[keys.length + 1]; 
    while (i < n)
      if (keys[i] != null) {
  ...
  public MaxPQ(Comparator<Key> comp, Key...keys) {
    comparator = comp;
    if (keys == null || keys.length == 0) {
      pq = (Key[]) new Object[1]; n = 0; return;
    }
...
//    par(toArray());
  ...
  public static void printHeaps(String[] s) {
    ...
    for (int i = 0; i < hp.length; i++) {
      MaxPQ<String> pq = new MaxPQ<String>(hq[i]);
      pq.printHeap();
    }     
  }
  ...
  public String toString() {
    ...
    for (int i = 1; i < n+1 ; i++) sb.append(pq[i]+",");
  ...
  public static void main(String[] args) {
    ...
    MaxPQ<Integer> pq = new MaxPQ<>(new Integer[]{6,5,4,3,2,1});
    par(pq.toArray());
