...
public class MaxPQex2409<Key> implements Iterable<Key> {
   ...
   */
  public MaxPQex2409(int initCapacity) {
   ...
   */
  public MaxPQex2409(int initCapacity, Comparator<Key> comparator) {
   ...
   * @param  comparator the order in which to compare the keys
   */
  public MaxPQex2409(Comparator<Key> comparator) {
    this(1, comparator);
  }
  ...
  public MaxPQex2409(Comparator<Key> comp, Key...keys) {
    comparator = comp;
    if (keys == null || keys.length == 0) {
      pq = (Key[]) new Object[1]; n = 0; return;
    }
...
//    par(toArray());
  ...
  private class HeapIterator implements Iterator<Key> {
    ...
    // create a new pq
    private MaxPQex2409<Key> copy;

    // add all items to copy of heap
    // takes linear time since already in heap order so no keys move
    public HeapIterator() {
      if (comparator == null) copy = new MaxPQex2409<Key>(size());
      else                    copy = new MaxPQex2409<Key>(size(), comparator);
      for (int i = 1; i <= n; i++)
        copy.insert(pq[i]);
    }
  ...
  public static void printHeaps(String[] s) {
    ...
    for (int i = 0; i < hp.length; i++) {
      MaxPQex2409<String> pq = new MaxPQex2409<String>(hq[i]);
      pq.printHeap();
    }     
  }
