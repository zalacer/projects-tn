...
public class TransitiveClosureEEWD {
...
//  private BagX<Integer>[] adj;
  ...
  public TransitiveClosureEEWD(EuclidianEdgeWeightedDigraph g) {
  ...
  public TransitiveClosureEEWD(EuclidianEdgeWeightedDigraph g, String noUpdate) {
    if (g == null) throw new IllegalArgumentException(
        "TransitiveClosureX: EdgeWeightedDigraphX is null");
    G = g; V = g.V(); //adj = g.adj();
    tc = new DirectedDFSX[V];
...
//    In in = new In(args[0]);
    EuclidianEdgeWeightedDigraph G = new EuclidianEdgeWeightedDigraph(6);
    G.addEdge(0,1,.19);  G.addEdge(1,2,.34);  G.addEdge(2,3,.27); 
    G.addEdge(3,4,.17); G.addEdge(4,5,.39);
    System.out.println("G:\n"+G);
    System.out.println("G.E="+G.E());
    int V = G.V();
    TransitiveClosureEEWD tc = new TransitiveClosureEEWD(G);
