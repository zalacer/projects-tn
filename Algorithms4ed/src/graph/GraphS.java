...
//implemented using Seq<Seq<Integer>> instead of BagX<Integer>[] for adj
...
public class GraphS {
  ...
  public GraphS() {
    ...
    V = E = 0;
    adj = new Seq<Seq<Integer>>(DEFAULTLEN);
    for (int i = 0; i < DEFAULTLEN; i++) adj.add(new Seq<Integer>());
  }
  ...
  public GraphS(int v) {
    ...
    V = v; E = 0;
    adj = new Seq<Seq<Integer>>(V);
    for (int i = 0; i < V; i++) adj.add(new Seq<Integer>());
  }
  
  public GraphS(int v, int e, int[][] x) {
  ...
  public GraphS(int v, String s) {
    ...
    E = 0; for (Seq<Integer> seq : adj) E += seq.size(); E /= 2;
    search();
  }
  
  public GraphS(String s) {
    ...
    for (Tuple2C<Integer,Integer> t : ta) {
      ...
      if (adj.size() < V) for (int i = V; i < 2*V; i++) adj.add(new Seq<Integer>());
      addEdge(t._1,t._2);      
    }
    if (V <= 0) return;
  ...
  public GraphS(In in) {
    try {
      ...
      adj = new Seq<Seq<Integer>>(V);
      for (int v = 0; v < V; v++) adj.add(new Seq<Integer>());
      int E = in.readInt();
  ...
  public Seq<Seq<Integer>> adj() { return adj.clone(); }
  
  public void setAdj(Seq<Seq<Integer>> seq) { adj = seq; }
  ...
  public void insertEdge(int v, int w) {
    ...
    if (max > V-1) V = max+1; 
    int s = adj.size();
    if (V > s) for (int i = 0; i < 2*V-s; i++) adj.add(new Seq<Integer>());
    adj.get(v).add(w);
    adj.get(w).add(v);
    E++;
  }
  ...
  public boolean removeEdge(int v, int w) {
    ...
    Seq<Integer> bv = adj.get(v); boolean fw = false; int iw = -1;  
    Seq<Integer> bw = adj.get(w); boolean fv = false; int iv = -1;
    int c = 0;
    for (Integer i : bv) {
