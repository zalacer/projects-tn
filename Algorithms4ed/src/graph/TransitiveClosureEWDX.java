...
public class TransitiveClosureEWDX {
...
//  private BagX<Integer>[] adj;
  ...
  public TransitiveClosureEWDX(EdgeWeightedDigraphX g) {
    if (g == null) throw new IllegalArgumentException(
        "TransitiveClosureX: EdgeWeightedDigraphX is null");
    G = g; V = g.V(); //adj = g.adj();
  ...
  public TransitiveClosureEWDX(EdgeWeightedDigraphX g, String noUpdate) {
    if (g == null) throw new IllegalArgumentException(
        "TransitiveClosureX: EdgeWeightedDigraphX is null");
    G = g; V = g.V(); //adj = g.adj();
    tc = new DirectedDFSX[V];
  ...
  public void updateGraph() {
    ...
    for (int i = 0; i < V; i++) 
      for (int j = 0; j < V; j++) 
        if (reachable(i,j)) {
          Iterable<DirectedEdgeX> itbl = G.adj(i);
          boolean contains = false;
          for (DirectedEdgeX e : itbl) if (e.to() == j) { contains = true; break; }
  ...
  public EdgeWeightedDigraphX graph() { return G; }

  public static void main(String[] args) {
//    In in = new In(args[0]);
    EdgeWeightedDigraphX G = new EdgeWeightedDigraphX(6);
    G.addEdge(0,1,.19);  G.addEdge(1,2,.34);  G.addEdge(2,3,.27); 
    G.addEdge(3,4,.17); G.addEdge(4,5,.39);
    System.out.println("G:\n"+G);
    System.out.println("G.E="+G.E());
    int V = G.V();
    TransitiveClosureEWDX tc = new TransitiveClosureEWDX(G);
