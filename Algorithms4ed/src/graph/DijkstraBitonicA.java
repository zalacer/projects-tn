...
public class DijkstraBitonicA {
  ...
  public DijkstraBitonicA(EdgeWeightedDigraphX g, int s) {
    ...
    insertions++;
    // find bito paths with edges sorted increasing for each vertex
    while (!pq.isEmpty()) {
      int v = pq.delMin();
  ...
  public static Seq<Seq<Seq<DirectedEdgeX>>> allBitonicPaths(EdgeWeightedDigraphX G) {
    ...
    for (int i = 0; i < G.V(); i++) r.add((new DijkstraBitonicA(G,i)).allPaths());
    return r;
  }
  ...
  public static void printDijkstraSPBitonic(EdgeWeightedDigraphX G, int source) {
    DijkstraBitonicA sp = new DijkstraBitonicA(G, source);
  ...
  public static Seq<Seq<Seq<Integer>>> allDijkstraSPBitonic(EdgeWeightedDigraphX G,
    ...
    DijkstraBitonicA D; Seq<Seq<Integer>> allp;
    for (int i = 0; i < V; i++) {
      D = new DijkstraBitonicA(G,i);
  ...
  public static void main(String[] args) {
    ...
    DijkstraBitonicA sp = new DijkstraBitonicA(G, s);
    par(sp.distTo());
