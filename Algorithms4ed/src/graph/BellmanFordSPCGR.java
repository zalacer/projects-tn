...
// BellmanFordSPX + the Cherkassky, Goldberg, and Radzik "parent-checking
// heurestic...that scans a vertex only if its parent isn't in the queue"
...
// This reference is available for download from:
...
public class BellmanFordSPCGR {
  ...
  public BellmanFordSPCGR(EdgeWeightedDigraphX g, int s) {
  ...
  private void relax(EdgeWeightedDigraphX G, int v) {
    if (edgeTo[v] != null && queue.contains(edgeTo[v].from())) {
      System.out.println("parent-checking heuristic on "+v);
      return;
    }
    relaxations++;
  ...
  public static final Seq<Seq<Seq<Integer>>> allPairsJohnson(EdgeWeightedDigraphX G,
    ...
    BellmanFordSPCGR B; Seq<DirectedEdgeX> edges;
    EdgeWeightedDigraphX H; DijkstraSPX D;
    Seq<Seq<Integer>> allp;
    for (int i = 0; i < V; i++) {
      B = new BellmanFordSPCGR(G,i);
...
//        allPairsJohnson(G);
        ...
        BellmanFordSPCGR sp = new BellmanFordSPCGR(G, s);
