...
//  private BagX<Integer>[] adj;
  ...
  public TransitiveClosureX(DigraphX g) {
  ...
  public TransitiveClosureX(DigraphX g, String noUpdate) {
    if (g == null) throw new IllegalArgumentException("TransitiveClosureX: DigraphX is null");
    G = g; V = g.V(); //adj = g.adj();
    tc = new DirectedDFSX[V];
    for (int v = 0; v < V; v++) tc[v] = new DirectedDFSX(g, v);
  ...
  public void updateGraph() {
    for (int i = 0; i < V; i++) 
      for (int j = 0; j < V; j++) 
        if (reachable(i,j) && !G.adj()[i].contains(j)) { G.addEdge(i,j); edgesAdded++; }
  }
  
  public int edgesAdded() { return edgesAdded; }
  
  public DigraphX graph() { return G; }

  public static void main(String[] args) {
//    In in = new In(args[0]);
    DigraphX G = new DigraphX(6);
