...
public class EdgeWeightedDigraphGeneratorX {
  
  private EdgeWeightedDigraphGeneratorX() {}
...
//        "toEdgeWeightedGraphX: input GraphX is null");
//    int V = G.V(), E = G.E();
...
//  }
  ...
  public static EdgeWeightedDigraphX toEdgeWeightedDigraphX(GraphX G) {
    // create and return an EdgeWeightedDigraphX with random positive weights 
    // from a GraphX
    // for each edge in G create forward and reverse edges with different weights
    if (G == null) throw new IllegalArgumentException(
        "toEdgeWeightedDigraphX: input GraphX is null");
    int V = G.V(), E = G.E();
    EdgeWeightedDigraphX g = new EdgeWeightedDigraphX(V);
    Seq<Tuple2<Integer,Integer>> edges = G.allEdges(); 
    SET<Double> dbls = new SET<>();
    int lim = (int)ceil(log10(2.*E))+1;
    double d = 0;
    StringBuilder sb = new StringBuilder();  
    ...
    for (Tuple2<Integer,Integer> t : edges) {
      ...
      dbls.add(d);
      g.addEdge(t._2,t._1,d); 
    }

    return g;
  }
  
  public static EdgeWeightedDigraphX toEdgeWeightedDigraphX(GraphX G, double w) {
    // create and return an EdgeWeightedDigraphX with constant positive weight w
    // from a GraphX
    // for each edge in G create forward and reverse edges with weight w in the
    if (G == null) throw new IllegalArgumentException(
        "toEdgeWeightedDigraphX: input GraphX is null");
    if (w < 0) throw new IllegalArgumentException(
        "toEdgeWeightedDigraphX: weight w is negative");
    int V = G.V();
    EdgeWeightedDigraphX g = new EdgeWeightedDigraphX(V);
    Seq<Tuple2<Integer,Integer>> edges = G.allEdges(); 
    for (Tuple2<Integer,Integer> t : edges) {
      g.addEdge(t._1,t._2,w);
      g.addEdge(t._2,t._1,w); 
    }
    return g;
  }
  
  public static EdgeWeightedDigraphX toEdgeWeightedDigraphXInc(GraphX G) {
    // create and return an EdgeWeightedDigraphX with constant positive weight w
    // from a GraphX
    // for each edge in G create forward and reverse edges with weight w in the
    if (G == null) throw new IllegalArgumentException(
  ...
  public static EdgeWeightedDigraphX toEdgeWeightedDigraphXDec(GraphX G) {
    // create and return an EdgeWeightedDigraphX with constant positive weight w
    // from a GraphX
    // for each edge in G create forward and reverse edges with weight w in the
    if (G == null) throw new IllegalArgumentException(
  ...
  public static EdgeWeightedDigraphX toEdgeWeightedDigraphXP1(GraphX G) {
    // create and return an EdgeWeightedDigraphX with constant positive weight w
    // from a GraphX
    // for each edge in G create forward and reverse edges with weight w in the
    if (G == null) throw new IllegalArgumentException(
    ...
    Seq<Tuple2<Integer,Integer>> edges = G.allEdges(); 
    for (Tuple2<Integer,Integer> t : edges) {
      g.addEdge(t._1,t._2,t._1+11*t._2);
      g.addEdge(t._2,t._1,t._2+11*t._1); 
    }
    return g;
  }
...
//  }
  ...
  public static EdgeWeightedDigraphX simple(int V, double p) {
    // return a random simple graph on V vertices, with an  edge between any two 
    // vertices with probability p (Erdos-Renyi random graph model) and unique
    // random weights
    if (p < 0.0 || p > 1.0)
    ...
    return toEdgeWeightedDigraphX(G);
  }
  
  public static EdgeWeightedDigraphX simple(int V, double p, double weight) {
    // return a random simple graph on V vertices, with an  edge between any two 
    // vertices with probability p (Erdos-Renyi random graph model) and weight w
    if (p < 0.0 || p > 1.0)
      throw new IllegalArgumentException("Probability must be between 0 and 1");
    GraphX G = new GraphX(V);
    for (int v = 0; v < V; v++)
      for (int w = v+1; w < V; w++)
        if (StdRandom.bernoulli(p)) G.addEdge(v, w);
    return toEdgeWeightedDigraphX(G,weight);
  }
  
  public static EdgeWeightedDigraphX simpleInc(int V, double p) {
    // return a random simple graph on V vertices, with an  edge between any two 
    // vertices with probability p (Erdos-Renyi random graph model) and weight w
    if (p < 0.0 || p > 1.0)
      throw new IllegalArgumentException("Probability must be between 0 and 1");
    ...
    for (int v = 0; v < V; v++)
      for (int w = v+1; w < V; w++)
        if (StdRandom.bernoulli(p)) G.addEdge(v, w);
    return toEdgeWeightedDigraphXInc(G);
  }
  
  public static EdgeWeightedDigraphX simpleDec(int V, double p) {
    // return a random simple graph on V vertices, with an  edge between any two 
    // vertices with probability p (Erdos-Renyi random graph model) and weight w
    if (p < 0.0 || p > 1.0)
      throw new IllegalArgumentException("Probability must be between 0 and 1");
    ...
    for (int v = 0; v < V; v++)
      for (int w = v+1; w < V; w++)
        if (StdRandom.bernoulli(p)) G.addEdge(v, w);
    return toEdgeWeightedDigraphXDec(G);
  }
  
  public static EdgeWeightedDigraphX simpleP1(int V, double p) {
    // return a random simple graph on V vertices, with an  edge between any two 
    // vertices with probability p (Erdos-Renyi random graph model) and weight w
    if (p < 0.0 || p > 1.0)
      throw new IllegalArgumentException("Probability must be between 0 and 1");
    ...
    for (int v = 0; v < V; v++)
      for (int w = v+1; w < V; w++)
        if (StdRandom.bernoulli(p)) G.addEdge(v, w);
    return toEdgeWeightedDigraphXP1(G);
  }

  public static EdgeWeightedDigraphX complete(int V) {
    // return the complete graph on V vertices
    return simple(V, 1.0);
  }
  
  public static EdgeWeightedDigraphX complete(int V, double weight) {
    // return the complete graph on V vertices
    return simple(V, 1.0, weight);
  }
  
  public static EdgeWeightedDigraphX completeInc(int V) {
    // return the complete graph on V vertices
    return simpleInc(V, 1.0);
  }
  
  public static EdgeWeightedDigraphX completeDec(int V) {
    // return the complete graph on V vertices
    return simpleDec(V, 1.0);
  }
  
  public static EdgeWeightedDigraphX completeP1(int V) {
    // return the complete graph on V vertices
    return simpleP1(V, 1.0);
  }
