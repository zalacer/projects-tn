...
public class RandomGridDigraph {
  
  public static SecureRandom sr = new SecureRandom();

  public static EuclidianDigraph completeGrid(int v) {
    // generates a complete v-by-v grid of directed edges connecting
  ...
  public static EuclidianDigraph randomGrid(int v) {
    // generates a random v-by-v grid with directed connecting adjacent edges 
    ...
    for (int i = 0; i < 119737; i++) sr.nextInt(2);
    while (it.hasNext()) {
    ...
    for (int i = 0; i < v; i++) {
      x = 1; y+=.5;
      for (int j = 0; j < v; j++) g.addCoords(k++,x+=.5,y);
    }
    g.trim(); // clear wasted memory in graph
    return g;
  }
  
  public static Tuple2<EuclidianDigraph,HashSET<Tuple2<Integer,Integer>>> 
    ...
    if (r > 0) {
      ...
      while (it.hasNext()) {
        ...
        p = Integer.parseInt(sa[0]);
        q = Integer.parseInt(sa[1]);
        if (g.hasEdge(q, p) || g.hasEdge(p, q)) continue;
        int u = sr.nextInt(2);
        if (u == 1) seq.add(new Tuple2<Integer,Integer>(p,q));
        else seq.add(new Tuple2<Integer,Integer>(q,p));
      }
      ...
      for (Tuple2<Integer,Integer> t : edges) {
        if (!(g.hasEdge(t._1, t._2) || g.hasEdge(t._2, t._1))) { 
          int u = sr.nextInt(2);
          if (u == 1) {
            g.addEdge(t._2, t._1);
            extraEdges.add(new Tuple2<Integer,Integer>(t._2, t._1));
          } else {
            g.addEdge(t._1, t._2);
            extraEdges.add(t);
          }
    ...
    g.trim(); // clear wasted memory in graph
    return new Tuple2<EuclidianDigraph,HashSET<Tuple2<Integer,Integer>>>(g,extraEdges);
  }
  
  public static Tuple3<EuclidianDigraph,HashSET<Tuple2<Integer,Integer>>,
    ...
    if (r > 0) {
      ...
      while (it.hasNext()) {
        ...
        p = Integer.parseInt(sa[0]);
        q = Integer.parseInt(sa[1]);
        if (g.hasEdge(p, q) || g.hasEdge(q, p)) continue;
        int u = sr.nextInt(2);
        if (u == 1) seq.add(new Tuple2<Integer,Integer>(q,p));
        else seq.add(new Tuple2<Integer,Integer>(p,q));
      } 
      // convert seq to array and shuffle
      Tuple2<Integer,Integer>[] edges = seq.to();
      shuffle(edges,sr);
      ...
      for (Tuple2<Integer,Integer> t2 : edges) {
        if (!(g.hasEdge(t2._1, t2._2) || g.hasEdge(t2._2, t2._1))) { 
          int u = sr.nextInt(2);
          if (u == 1) {
            g.addEdge(t2._2, t2._1); c++; 
            extraEdges.add(new Tuple2<Integer,Integer>(t2._2, t2._1));
          } else {
    ...
    if (b == true) {
      ...
      // scale the normalization factor f multiplicatively up by 100 to avoid 
      // overclouding the plot
      f = 100./minDist; 
      for (int i = 0; i < V; i++)
        for (int j = i+1; j < V; j++){
          double w = 1./(f*distance(coords.get(i),coords.get(j)));
          if (!(w >= 0.0 && w <= 1.0)) continue;
          if (sr.nextDouble() < w) { // bernoulli criterion
            int u = sr.nextInt(2);
            if (u == 1) {
              g.addEdge(j,i);
              pEdges.add(new Tuple2<Integer,Integer>(j,i));   
            } else {
              g.addEdge(i,j);
    ...
    g.trim(); // clear wasted memory in graph
    return new Tuple3<EuclidianDigraph,HashSET<Tuple2<Integer,Integer>>,HashSET<Tuple2<Integer,Integer>>>(g,extraEdges,pEdges);
  }
  ...
  public static void demo() {
   EuclidianDigraph g;
    ...
    Tuple2<EuclidianDigraph,HashSET<Tuple2<Integer,Integer>>> t2 = randomGrid(15,19);
    g = t2._1;
    ...
    Tuple3<EuclidianDigraph,HashSET<Tuple2<Integer,Integer>>,
