...
// with arbitrary edge weights (independent of euclidian distance between vertices)
// for ex4433

public class DijkstraSourceSinkSPEEWD {
  ...
  public DijkstraSourceSinkSPEEWD(EuclidianEdgeWeightedDigraph E, int s, int t) {
  ...
  public void printSouceSinkPath() {
    for (int t = 0; t < G.V(); t++) {
      ...
      if (this.hasPathTo(t)) {
          ...
          System.out.println(str(pathTo(t)));
      }
  ...
  public String str(Iterable<DirectedEdgeX> s) { return convert(s).mkString(">"); }
  
  public int sumOfUniqueVertices(Iterable<DirectedEdgeX> edges) {
    // for ex4425, but not needed
    return Seq.fold(convert(edges), (x,y) -> x+y, 0); 
  }
  ...
  public static void sourceSink(String EdgeWeightedDigraphXfileName, 
    ...
    DijkstraSourceSinkSPEEWD sp = new DijkstraSourceSinkSPEEWD(E, source, sink);
