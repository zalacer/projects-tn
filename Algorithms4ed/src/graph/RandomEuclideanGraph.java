...
public class RandomEuclideanGraph {
  ...
  public static EuclidianGraph createRandomEuclidianGraph(int v, boolean connected) {
    SecureRandom r = new SecureRandom(); r.setSeed(System.currentTimeMillis());
    for (int i = 0; i < 130002; i++) r.nextDouble();
    ...
    while (p.test(count)) {
      ...
      for (int x = 0; x < v; x++)
        for (int y = x+1; y < v; y++) 
          if (connected) {
            if (distance(ta[x],ta[y]) > m) g.addEdge(x,y);
          } else if (distance(ta[x],ta[y]) < m) g.addEdge(x,y);
  ...
  public static void main(String[] args) {
    ...
    EuclidianGraph g = createRandomEuclidianGraph(5,connected);
    g.show();
