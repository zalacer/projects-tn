...
//    }
  ...
  private boolean hasSelfLoop(GraphX G) {
    for (int v = 0; v < G.V(); v++) {
      for (int w : G.adj(v)) {
        if (v == w) {
  ...
  private boolean hasParallelEdges(GraphX G) {
    marked = new boolean[G.V()];
    for (int v = 0; v < G.V(); v++) {
  ...
  public boolean findAllParallelEdges(GraphX G) {
    marked = new boolean[G.V()]; 
    ...
    for (int v = 0; v < G.V(); v++) {
      ...
      for (int w : G.adj(v)) {
        ...
        if (marked[w] && v != w && !done.contains(s1) && !done.contains(s2)) {
          ...
          cycle.push(w);
          cycle.push(v);
          pe.add(cycle); pearr.add((int[])unbox(cycle.toArray(1)));
  ...
  public int[] parallelEdgeSequence() {
    // for determining GraphX isomorphisms
    if (pearr == null) return new int[0];
    int[] cs = new int[pearr.size()]; int c = 0;
    for (int[] a : pearr) cs[c++] = a.length;
    Arrays.sort(cs);
    return reverse(cs);
  }
  ...
  public static void main(String[] args) {
    ...
    System.out.println("fapecount="+finder.getFapecount());
    par(G.adj());
//    if (finder.hasCycle()) {
