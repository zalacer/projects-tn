...
// implemented using GraphSB

public class BreadthFirstPathsSB {
  ...
  public BreadthFirstPathsSB(GraphSB G, int s) {
...
//    System.out.println("check returned "+chk+"\n");
  ...
  public BreadthFirstPathsSB(GraphSB G, int s, String type) {
  ...
  public BreadthFirstPathsSB(GraphSB G, Iterable<Integer> sources) {
  ...
  private void bfs(GraphSB G, int s) {
  ...
  private void bfstack(GraphSB G, int s) {
  ...
  private void bfs(GraphSB G, Iterable<Integer> sources) {
  ...
  private void bfstack(GraphSB G, Iterable<Integer> sources) {
    // for Ex4114 - using a stack instead of a queue in bfs
  ...
  private boolean check(GraphSB G, int s) {
//    System.out.println("check() messages:");
  ...
  public static void bfSearch(String file, int source) {
    ...
    GraphSB G = new GraphSB(new In(file));
    BreadthFirstPathsSB bfs = new BreadthFirstPathsSB(G,source);
  ...
  public static void bfSearch(String file, int source, String type) {
    ...
    if (!(type.equals("q")||type.equals("s"))) throw new IllegalArgumentException(
        "BreadthFirstPathsX: type isn't \"q\" or \"s\"");
    GraphSB G = new GraphSB(new In(file));
    BreadthFirstPathsSB bfs = null;
    if (type.equals("s")) bfs = new BreadthFirstPathsSB(G,source, "s");
    else bfs = new BreadthFirstPathsSB(G,source);
    StdOut.println("Paths:");
    for (int v = 0; v < bfs.V(); v++) {
      if (bfs.hasPathTo(v)) {
        StdOut.printf("%d to %d (%d):  ", source, v, bfs.distTo(v));
...
//    bfSearch("tinyGex2.txt",0,"s");
    ...
    In in = new In(args[0]);
    GraphSB G = new GraphSB(in);
    System.out.print("G.adj="+G.adj());

    int s = Integer.parseInt(args[1]);
    BreadthFirstPathsSB bfs = new BreadthFirstPathsSB(G, s);
