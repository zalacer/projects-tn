...
public class TopologicalBFSWrong {
  ...
  public TopologicalBFSWrong(DigraphX G) {
    ...
    for (int v = 0; v < G.V(); v++) { queue.enqueue(v); marked[v] = true; }
    while (!queue.isEmpty()) {
      int v = queue.dequeue();
      order.enqueue(v);
      ranks[v] = count++;
  ...
  public TopologicalBFSWrong(EdgeWeightedDigraphX G) {
    int[] indegree = new int[G.V()];
    for (int v = 0; v < G.V(); v++) indegree[v] = G.indegree(v);
    ranks = new int[G.V()]; 
    order = new Queue<Integer>();
  ...
  public static void main(String[] args) {
    ...
    TopologicalBFSWrong topological1 = new TopologicalBFSWrong(G1);
    ...
    // find a directed cycle
    TopologicalBFSWrong topological2 = new TopologicalBFSWrong(G2);
