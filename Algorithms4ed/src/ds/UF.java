...
/*
 * from http://algs4.cs.princeton.edu/code/edu/princeton/cs/algs4/UF.java
...
public class UF {
  ...
  public int find(int p) {
    ...
    while (p != parent[p]) {
      ...
      p = parent[p];
      parentArrayAccesses+=5; // counting conservatively however parent[p] would
    }                         // probably be accessed only once per loop in compiled code   
    parentArrayAccesses++;
    return p;
  }
...
//      System.out.println("uf.find(5)= "+uf.find(5));
//      uf.union(p, q);
...
//      StdOut.println(p + " " + q);
    ...
    System.out.println(t.elapsed()); //3612 ms for largeUF.txt, 31ms for tinyUF.txt
  }
}
