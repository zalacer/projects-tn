...
public class LinkedList<T> implements Iterable<T>, Cloneable {
  ...
  public boolean remove(T t) {
    ...
    Node<T> node = first, prev;
    if (node.item.equals(t)) { removeFirst(); return true; }
    while (node.next != null) {
      prev = node;
      node = node.next();
  ...
  public Node<T> remove(int k) {
    ...
    if (k < 0) throw new IllegalArgumentException("remove: k must be >= 0");
    if (isEmpty()) return null;
//    if (isEmpty()) throw new NoSuchElementException("remove: LinkedList underflow");
    if (size()-1 < k) 
      throw new IndexOutOfBoundsException("remove: LinkedList has max index "+(k-1));
  ...
  public Node<T> removeAfter(Node<T> node) {
    // removes the Node after node except if node.next is null
