...
public class ResizingArray<T> implements Iterable<T> {
  ...
  public ResizingArray(ResizingArray<T> bqos) {
  ...
  public void add(int index, T t) { // inserts t at index
    ...
    if (index > last-1) throw new IndexOutOfBoundsException();
    if (N == a.length) resize(2*a.length); 
    for (int i = last-1; i >= index; i--) a[i+1] = a[i];
    a[index] = t;
    last = (last + 1) % a.length; // handles wrap-around
    N++;
  }
  ...
  public boolean equals(Object obj) {
    ...
    ResizingArray other = (ResizingArray) obj;
    if (N != other.N)
      return false;
