...
public class ResizingArrayQueue<T> implements Iterable<T> {
  ...
  public ResizingArrayQueue(T[] sa) {
    ...
    this.a = (T[]) new Object[sa.length];
    for (int i = 0; i < sa.length; i++) if (sa[i] != null) enqueue(sa[i]);
//    a = sa;
//    last = a.length;
//    N = last;
  }
  ...
  public T kth(int k) {
    // return the kth from first element in the queue where 1st = first
    // this solution uses a sliding window implemented with an array
    if (k < 0) throw new IllegalArgumentException("kth: k must be > -1");
    if (this.size() < k) throw new NoSuchElementException("kth from last element does not exist");
    return a[(first+k) % a.length];
  }
