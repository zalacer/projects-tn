...
public class Digraph<V> {
  ...
  public Map<V,List<V>> neighbors() { return neighbors; }
  
  public int V() { return neighbors.size(); }
  
  public int E() {
    if (V() == 0) return 0;
    int e = 0;
    for (V v : neighbors.keySet()) e += neighbors.get(v).size();
    return e;
  }
  ...
  public void add (V vertex) {
    if (vertex == null) throw new IllegalArgumentException("add: vertex V is null");
    if (vclass == null) vclass = vertex.getClass();
  ...
  public List<V> topSort () {
    ...
    for (V v: degree.keySet()) if (degree.get(v) == 0) zeroVerts.push(v);
   ...
   */
  public boolean isDag () { return topSort() != null; }
