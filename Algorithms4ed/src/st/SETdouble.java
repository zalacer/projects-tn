...
// based on st.STint for ex3507

public class SETdouble implements OrderedSETdouble {
  ...
  public SETdouble(double[] ka) {
    if (ka == null || ka.length == 0) return;
    int n = ka.length;
    for (int  i = 0; i < n; i++) add(ka[i]);
  }
  ...
  private Node add(Node h, double key) { 
  ...
  @Override public Iterator<Double> iterator() { return keys().iterator(); }
