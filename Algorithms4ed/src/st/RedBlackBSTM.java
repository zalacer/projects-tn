...
// based on RedBlackBSTX to allow duplicate keys for ex3510 while retaining
// only the most recent value for each key.
// for ex3510.

public class RedBlackBSTM<Key extends Comparable<Key>, Value> {
  ...
  private class Node {
    ...
    private int c;             // count of Values for key == #Keys == k
    private Node left, right;  // links to left and right subtrees
    private boolean color;     // color of parent link
    private int size;          // subtree count
    private int dSize;         // number of nodes in subtree including duplicates

    public Node(Key key, Value val, boolean color, int c, int size, int dSize) {
      this.key = key;
      this.val = val;
      this.color = color;
      this.c = c;
    ...
    public String toString() {
      return "("+key+","+val+","+c+","+size+","+dSize+")";
    }
  }

  public RedBlackBSTM(){}
  
  public RedBlackBSTM(Key[] ka, Value[] va) {
  ...
  public int size(Node x) { if (x == null) return 0; return x.size; }
  ...
  private Node getNode(Node x, Key key) {
    if (root == null || x == null || key == null) return null;
    while (x != null) {
      int cmp = key.compareTo(x.key);
      if      (cmp < 0) x = x.left;
      else if (cmp > 0) x = x.right;
  ...
  private Node put(Node h, Key key, Value val) { 
    if (h == null) return new Node(key, val, RED, 1, 1, 1);
  ...
  private void allKeys(Node x, Queue<Key> queue, Key lo, Key hi) { 
    ...
    if (cmplo <= 0 && cmphi >= 0) for (int i = 0; i < x.c; i++) queue.enqueue(x.key); 
...
//        && isDsizeConsistent() && isBalanced();
    return isBST() && isSizeConsistent() && isRankConsistent() && isDsizeConsistent()
        && isDrankConsistent() && is23() && isBalanced();
  }
  ...
  public boolean isDrankConsistent() {
    ...
    for (Key key : keys()) {
      if (key.compareTo(dSelect(dRank(key))) != 0) return false;
    }
    return true;
  }
  ...
  public RedBlackBSTM<Integer,Integer> pathLengths() { return pathLengths(root); }
    
  public RedBlackBSTM<Integer,Integer> pathLengths(Node node) {
    List<Key> list = new ArrayList<>();
    RedBlackBSTM<Integer,Integer> map = new RedBlackBSTM<>();
    pathLengths(node, list, map, 0);
    return map;
  }
  
  private void pathLengths(Node node, List<Key> list, 
      RedBlackBSTM<Integer,Integer> map, int pathLen) {
    if (node == null) return;
  ...
  public Key[] toInOrderArray() {
    ...
    Iterator<Key> it = keys().iterator(); int c = 0;
