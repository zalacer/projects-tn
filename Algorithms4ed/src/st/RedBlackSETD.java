...
// based on RedBlackBSTX with Double keys and no values, for HashList.

public class RedBlackSETD {
  ...
  public RedBlackSETD(Double...z) {
    if (z == null) return;
    for (int  i = 0; i < z.length; i++) put(z[i]);
  }
  ...
  private boolean get(Node x, Double key) {
    while (x != null) {
  ...
  public boolean contains(Double key) { return get(key); }
...
//    else              h.val   = val;
  ...
  public Double[] toArray() { return v.ArrayUtils.toArray(keys().iterator()); }
  ...
  public Double[] toArray(Double lo, Double hi) { 
    return v.ArrayUtils.toArray(keys(lo,hi).iterator()); }

  public Double[] toArray(int lo, int hi) { 
    return v.ArrayUtils.toArray(keys(select(lo),select(hi)).iterator()); }
  ...
  public static void main(String[] args) {
    ...
    Timer t = new Timer();
    RedBlackSETD set = new RedBlackSETD(a);
