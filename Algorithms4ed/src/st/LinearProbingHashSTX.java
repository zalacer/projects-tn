...
public class LinearProbingHashSTX<Key, Value> {
  ...
  private Value[] vals = null; // the values
  ...
  public LinearProbingHashSTX() { this(INIT_CAPACITY); }
  
  public LinearProbingHashSTX(boolean noresize) { this(INIT_CAPACITY,noresize); }
  ...
  public LinearProbingHashSTX(String s) {
  ...
  public LinearProbingHashSTX(int capacity, String s) {
    if (capacity < 1) throw new IllegalArgumentException(
  ...
  public LinearProbingHashSTX(int capacity, boolean noresize) {
    if (capacity < 1) throw new IllegalArgumentException(
  ...
  public LinearProbingHashSTX(int capacity, String s, boolean noresize) {
    if (capacity < 1) throw new IllegalArgumentException(
  ...
  public LinearProbingHashSTX(int capacity, int p) {
    if (capacity < 1) throw new IllegalArgumentException(
  ...
  public LinearProbingHashSTX(int capacity, int p, boolean noresize) {
    this(capacity,p);
    this.noresize = noresize;
  }

  public LinearProbingHashSTX(Key[] ka, Value[] va) {
  ...
  public LinearProbingHashSTX(Key[] ka, Value[] va, String s) {
    if (ka == null || va == null || ka.length == 0 || va.length == 0) return;
    int len = Math.min(ka.length, va.length); int c = 0;
    Tuple2<Key,Value> ta[] = ofDim(Tuple2.class,len);
    for (int i = 0; i < len; i++) 
...
//        m = 4*m; ma[0] = m;
  ...
  public LinearProbingHashSTX(Key[] ka, Value[] va, String s, boolean noresize) {
    if (ka == null || va == null || ka.length == 0 || va.length == 0) return;
    int len = Math.min(ka.length, va.length); int c = 0;
    Tuple2<Key,Value> ta[] = ofDim(Tuple2.class,len);
    for (int i = 0; i < len; i++) 
...
//        m = 4*m; ma[0] = m;
  ...
  public LinearProbingHashSTX(Key[] ka, Value[] va, boolean noresize) {
    if (ka == null || va == null || ka.length == 0 || va.length == 0) return;
    int len = Math.min(ka.length, va.length); int c = 0;
    Tuple2<Key,Value> ta[] = ofDim(Tuple2.class,len);
    for (int i = 0; i < len; i++) 
  ...
  public LinearProbingHashSTX(Key[] ka, Value[] va, int p) {
    if (p < 1) throw new IllegalArgumentException(
    ...
    kclass = ka.getClass().getComponentType();
    vclass = va.getClass().getComponentType();
    keys = ofDim(kclass, m); vals = ofDim(vclass, m);
    pa[0] = p;
    hash = (k) -> {
  ...
  public LinearProbingHashSTX(Key[] ka, Value[] va, int p, boolean noresize) {
    if (p < 1) throw new IllegalArgumentException(
        "LinearProbingHashSTX(Key[],Value[],int) constructor 3rd arg < 1");
    if (ka == null || va == null || ka.length == 0 || va.length == 0) return;
    int len = Math.min(ka.length, va.length); int c = 0;
  ...
  public void setHash(Object p) {
    ...
    Key[] ka = keys;
    Value[] va = vals;
    keys = ofDim(kclass, m); vals = ofDim(vclass,m); n = 0;
    for (int i = 0; i < ka.length; i++) 
      if (ka[i] != null) put(ka[i], va[i]);
  }
...
//    for (int i : cl) sum += cl[i]*cl[i];
  ...
  private void resize(int capacity) {
    ...
    LinearProbingHashSTX<Key, Value> temp;
    if (s != null && !s.equals("dhash")) {
      temp = new LinearProbingHashSTX<>(adjcapacity,"dhash");
    } else if (s != null && !s.equals("dhash2")) {
      temp = new LinearProbingHashSTX<>(adjcapacity,"dhash2");
    } else temp = new LinearProbingHashSTX<>(adjcapacity);
    for (int i = 0; i < m; i++)
      if (keys[i] != null) {
        resizeEquals++;
  ...
  public void put(Key key, Value val) {
    ...
    if (vals == null) vals = ofDim(vclass, m);
    if (n >= m/2) resize(2*m);
    int i, ptmp = 0;
    if (s != null && s.equals("dhash")) {
  ...
  public void delete(Key key) {
    ...
    while (keys[i] != null) {
      rehashEquals++;
      Key keyToRehash = keys[i];  Value valToRehash = vals[i];
  ...
  public Value[] getVals() { return vals; }
  ...
  public void showKeys() {
    if (keys == null) { System.out.println("empty"); return; }
  ...
  public static void main(String[] args) { 
    ...
    Timer t = new Timer();
    SecureRandom r = new SecureRandom();
    Double[] a = rangeDouble(1.,1000.);
    shuffle(a,r);
    Integer[] b = rangeInteger(1,1000);
    t = new Timer();
    LinearProbingHashSTX<Double,Integer> st = new LinearProbingHashSTX<>(a,b);
    ...
    LinearProbingHashSTX<Character, Integer> ht = new LinearProbingHashSTX<>(ca,ia);
    ht.show(); // Q:4 S:2 T:6 A:1 U:5 Y:3 E:0 I:7 N:9 O:8
    ht.setHash(11);
    ht.show(); // Y:9 I:2 T:7 O:4 Q:5 S:6 A:0 U:8 N:3 E:1 
    ...
    //    par(ht.toKeyArray());
    //    par(ht.toValArray());
    //    par(ht.toUnsortedKeyArray());
    //    par(ht.toUnsortedValArray());
    //    Iterator<String> it = ht.keys().iterator();
