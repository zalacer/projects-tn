...
public class BinarySearchSTex3125<Key extends Comparable<Key>, Value> {
    private static final int INIT_CAPACITY = 2;
    private Key[] keys;
    private Value[] vals;
    private int n = 0;
    private int compares = 0;
    private Tuple2<Key,Integer> recent = null;

    public BinarySearchSTex3125() { this(INIT_CAPACITY); }

    public BinarySearchSTex3125(int capacity) { 
        keys = (Key[]) new Comparable[capacity]; 
        vals = (Value[]) new Object[capacity]; 
    }
    
    public BinarySearchSTex3125(Key[] ka, Value[] va) {
    ...
    public Value get(Key key) {
        if (key == null) throw new NullPointerException();
        if (recent != null && recent._1.equals(key)) return vals[recent._2];
        if (isEmpty()) return null;
        int i = rank(key); 
        if (i < n && keys[i].compareTo(key) == 0) {
          recent = new Tuple2<Key,Integer>(key,i);
          return vals[i];
        }
        return null;
    } 
    ...
    public void put(Key key, Value val)  {
        ...
        if (i < n && keys[i].compareTo(key) == 0) { 
          compares++; vals[i] = val; 
          if (recent != null && recent._1.equals(key)) recent._2 = i;
          return; 
        }
        compares++;
        if (n == keys.length) resize(2*keys.length);
        for (int j = n; j > i; j--) { keys[j] = keys[j-1]; vals[j] = vals[j-1]; }
        if (recent != null && recent._2 > i) recent._2++;
    ...
    public void delete(Key key) {
        ...
        if (recent._2.equals(i)) recent = null;
        else if (recent._2 > i) recent._2--;
