...
//all values for each key in a separate Queue.
//for ex3521.

public class RedBlackBSTQ<Key extends Comparable<Key>, Value> {
  ...
  private class Node {
    private Key key; // key
    private Queue<Value> q; // queue to contain all values
    ...
    public Node(Key key, Queue<Value> q, boolean color, int c, int size, int dSize) {
      this.key = key;
      this.q = q;
      this.color = color;
      this.c = c;
    ...
    public String toString() {
      return "(" + key + "," + arrayToString(q.toArray(), 90, 1, 1) + "," + c + "," + size + "," + dSize + ")";
    }
  }
  ...
  public RedBlackBSTQ(Key[] ka, Value[] va) {
  ...
  public Value[] getValues(Key key) {
    ...
    if (q == null)
      return null;
    if (vclass != null)
      return q.toArray((Class<? extends Value>) vclass);
    return (Value[]) q.toArray();
  }

  public Queue<Value> getQueue(Key key) {
    if (key == null)
      throw new IllegalArgumentException("argument to get() is null");
    return get(root, key);
  }
  ...
  private Node put(Node h, Key key, Value val) {
    if (h == null) {
      Node nd = new Node(key, new Queue<Value>(), RED, 1, 1, 1);
      nd.q.enqueue(val);
      return nd;
    }
  ...
  public Tuple2<Key,Value>[] rangeLookup(Key a, Key b) {
    ...
    Queue<Tuple2<Key,Value>> q = new Queue<>();
    for (int i = 0; i < ka.length; i++) {
      Key k = ka[i];
      Node n = getNode(k);
      Value[] va = n.q.toArray((Class<? extends Value>)vclass);
      for (int j = 0; j < va.length; j++) {
        q.enqueue(new Tuple2<Key,Value>(k,va[j]));
      }
    }
    return q.toArray((Class<? extends Tuple2<Key,Value>>)Tuple2.class);
  }
  ...
  public RedBlackBSTQ<Integer, Integer> pathLengths() {
    return pathLengths(root);
  }

  public RedBlackBSTQ<Integer, Integer> pathLengths(Node node) {
    List<Key> list = new ArrayList<>();
    RedBlackBSTQ<Integer, Integer> map = new RedBlackBSTQ<>();
    pathLengths(node, list, map, 0);
    return map;
  }

  private void pathLengths(Node node, List<Key> list, RedBlackBSTQ<Integer, Integer> map, int pathLen) {
    if (node == null)
      return;
  ...
  public static void redBlackTreeWorstCaseTest(int size) {
    ...
    int n = size;
    RedBlackBSTQ<String, Tuple3<Integer, Integer, Integer>> mapr = new RedBlackBSTQ<>();
    ...
    while (it.hasNext()) {
      int[] d = it.next();
      c = (Integer[]) box(d);
      RedBlackBSTQ<Integer, Integer> st = new RedBlackBSTQ<>(c, b);
      RedBlackBSTQ<Integer, Integer> map = st.pathLengths();
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < sa.length; i++)
      ca[i] = sa[i].charAt(0);
    Integer[] ia = rangeInteger(0, sa.length);
    RedBlackBSTQ<Character, Integer> h = new RedBlackBSTQ<>(ca, ia);
    System.out.print("h = " + h.tos() + "\n");
