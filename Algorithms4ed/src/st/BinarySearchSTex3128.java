...
public class BinarySearchSTex3128<Key extends Comparable<Key>, Value> {
    private static final int INIT_CAPACITY = 2;
    private Key[] keys;
    private Value[] vals;
    private int n = 0;
    private int compares = 0;
    private Key max = null;

    public BinarySearchSTex3128() { this(INIT_CAPACITY); }
    ...
    public BinarySearchSTex3128(Key[] ka, Value[] va) {
      if (ka == null || va == null || ka.length == 0 || va.length == 0) return;
      int m = Math.min(ka.length, va.length); int c = 0; 
      Key[] kz = ofDim(ka.getClass().getComponentType(), m);
      for (int  i = 0; i < m; i++) {
        if (c == m) break;
      ...
      if (c < m) { m = c; kz = take(kz,c); }
      Value[] vz = ofDim(va.getClass().getComponentType(), m); c = 0;
      for (int  i = 0; i < m; i++) {
        if (c == m) break;
      ...
      if (c < m) { m = c; kz = take(kz,c); vz = take(vz,c); }
      keys = (Key[]) new Comparable[2*m];
      vals = (Value[]) new Object[2*m]; 
      //ex3128 constant timed ordered insertions when possible
      for (int  i = 0; i < m; i++) { 
        if (max == null || kz[i].compareTo(max) > 0) {
          max = kz[i]; keys[n] = kz[i]; vals[n] = vz[i]; n++; continue;
        } else put(kz[i], vz[i]);
      }
    }
    ...
    public void put(Key key, Value val)  {
        ...
        //resize early for constant time insertions
        if (n == keys.length) resize(2*keys.length);
        //ex3128 constant timed ordered insertions when possible
        if (max == null || key.compareTo(max) > 0) { 
          max = key; keys[n] = key; vals[n] = val; n++; 
          assert check(); return;
        }
        int i = rank(key);
        if (i < n && keys[i].compareTo(key) == 0) { 
          compares++; vals[i] = val; return; 
        }
        compares++;
...
//      System.out.println(sb.toString());
      ...
      BinarySearchSTex3128<String, Integer> st = new BinarySearchSTex3128<>();
      for (int  i = 0; i < keys.length; i++) st.put(keys[i], values[i]);
      System.out.println(st);
      System.out.println(st.size()+"\n");
      
      st = new BinarySearchSTex3128<>(keys,values);
      System.out.println(st);
      System.out.println(st.size()+"\n");
