...
public class STdouble implements OrderedSTdouble {
  ...
  private class Node {
    ...
    public Node(double key, double val, boolean color, int size) {
      this.key = key;
      this.val = val;
      this.color = color;
      this.size = size;
  ...
  public STdouble(double[] ka, double[] va) {
  ...
  @Override public boolean contains(double key) { return get(key) != null; }

  @Override public void put(double key, double val) {
  ...
  private Node put(Node h, double key, double val) { 
  ...
  @Override public Double floor(double key) {
  ...
  @Override public Double ceiling(double key) {
  ...
  @Override public double select(double k) {
  ...
  private Node select(Node x, double k) {
    // assert x != null;
    // assert k >= 0 && k < size(x);
    int t = size(x.left); 
  ...
  @Override public int rank(double key) {
  ...
  @Override public Iterable<Double> keys() {
    if (isEmpty()) return new Queue<Double>();
    return keys(min(), max());
  }

  @Override public Iterable<Double> keys(double lo, double hi) {
  ...
  @Override public int size(double lo, double hi) {
    if (lo - hi > 0) return 0;
