...
public class SequentialSearchSTex3125<Key, Value> {
  private int n; 
  private Node first;
  ...
  public SequentialSearchSTex3125(){}
  
  public SequentialSearchSTex3125(Key[] ka, Value[] va) {
  ...
  public Value get(Key key) {
    if (key == null) throw new NullPointerException();
    if (recent != null && recent.key.equals(key)) { equals++; return recent.val; }
    for (Node x = first; x != null; x = x.next)
      if (key.equals(x.key)) { recent = x; return x.val; }
    return null;
  }
  ...
  public void put(Key key, Value val) {
    ...
    for (Node x = first; x != null; x = x.next) {
      if (key.equals(x.key)) { 
        x.val = val; equals++;
        if (recent != null) {
          if (recent.key.equals(key)) recent.val = val;
          equals++;
        }
        return; 
      }
  ...
  private Node delete(Node x, Key key) {
    ...
    if (key.equals(x.key)) {
      if (recent != null && recent == x) recent = null;
      n--; return x.next; }
...
//    if (n == 1) return sb.toString();
  ...
  public static void main(String[] args) {
    ...
    SequentialSearchSTex3125<String,Integer> st = new SequentialSearchSTex3125<>(k,v);
