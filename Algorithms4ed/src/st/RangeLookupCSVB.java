package st;

import static v.ArrayUtils.*;

import java.io.File;
import java.io.FileNotFoundException;
import java.util.Iterator;
import java.util.Scanner;

// based on LookupCSV modified for possible multiple values per key for ex3512

import st.RedBlackBSTB;

public class RangeLookupCSVB {

  // Do not instantiate.
  private RangeLookupCSVB() { }

  public static void main(String[] args) throws FileNotFoundException {
    RedBlackBSTB<String, String> st = new RedBlackBSTB<>();
    Scanner sc = new Scanner(new File(args[0]));
    int kf = Integer.parseInt(args[1]);
    int vf = Integer.parseInt(args[2]);
    while (sc.hasNextLine()) {
      String line = sc.nextLine();
      String[] tokens = line.split(",");
      String k = tokens[kf]; 
      String v = tokens[vf];
      st.put(k, v);
    }
    sc.close();
 
    System.out.println("verifying input from "+args[0]+":");
    System.out.println("st = "+st.tos());
    System.out.println("st.size() = "+st.size());
    Iterator<String> it = st.keys().iterator();
    System.out.println("key  values");
    while (it.hasNext()) {
      String k = it.next(); 
      System.out.print(k+"    "); par(st.getValues(k)); 
    }

    System.out.println("\nenter a pair of keys for range lookup:");
    sc = new Scanner(System.in);
    while (sc.hasNext()) {
      String k1 = sc.next();
      String k2 = sc.next();
      if (st.contains(k1) && st.contains(k2))
        { System.out.print("rangeLookup("+k1+","+k2+") = "); par(st.rangeLookup(k1, k2)); }
      else {
        if (!st.contains(k1)) System.out.println(k1+" key not found");
        if (!st.contains(k2)) System.out.println(k2+" key not found");
      }
      System.out.println("\nenter another pair of keys for range lookup:");
    }
    sc.close();

    //        while (!StdIn.isEmpty()) {
    //            String s = StdIn.readString();
    //            if (st.contains(s)) st.geAtQueue(s).show();
    //            else                System.out.println("Not found");
    //        }
  }

  //*  % java LookupCSV amino.csv 0 3     % java LookupCSV ip.csv 0 1 
  //*  TTA                                www.google.com 
  //*  Leucine                            216.239.41.99 
  //*  ABC                               
  //*  Not found                          % java LookupCSV ip.csv 1 0 
  //*  TCT                                216.239.41.99 
  //*  Serine                             www.google.com 
  //*                                 
  //*  % java LookupCSV amino.csv 3 0     % java LookupCSV DJIA.csv 0 1 
  //*  Glycine                            29-Oct-29 
  //*  GGG                                252.38 
  //*                                     20-Oct-87 
  //*                                     1738.74
}
