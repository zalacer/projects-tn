...
public class LinkedList<Item> implements Iterable<Item> {
  ...
  public LinkedList(LinkedList<Item>.Node x) {
    this.first = x;
    if (first == null) {
      this.last = null;
      N = 0;
      ...
      N++;
      LinkedList<Item>.Node node = first;
...
//      item = node.item;
  ...
  public static <T> LinkedList<T>.Node iterativeReverse(LinkedList<T>.Node x) {
    LinkedList<T>.Node first = x;
  ...
  public static <T> LinkedList<T>.Node recursiveReverse(LinkedList<T>.Node first) {
    if (first == null) return null;
    if (first.next == null) return first;
    LinkedList<T>.Node second = first.next;
    LinkedList<T>.Node rest = recursiveReverse(second);
    second.next = first;
    first.next = null;
    return rest;
  }
