...
// for ex2429 dev - added extra methods used in
...
public class MaxPQex2429<Key> implements Iterable<Key> {
   ...
   */
  public MaxPQex2429(int initCapacity) {
   ...
   */
  public MaxPQex2429(int initCapacity, Comparator<Key> comparator) {
   ...
   * @param  comparator the order in which to compare the keys
   */
  public MaxPQex2429(Comparator<Key> comparator) {
    this(1, comparator);
  }
  ...
  public MaxPQex2429(Comparator<Key> comp, Key...keys) {
    comparator = comp;
    if (keys == null || keys.length == 0) {
      pq = (Key[]) new Object[1]; n = 0; return;
    }
  ...
  // based on delMax()
  public Key removeAtIndex(int v) {
    if (isEmpty()) throw new NoSuchElementException("Priority queue underflow");
    Key k = pq[v];
    exch(v, n--);
    Key l = pq[v];
    sink(v);
//    show();
//    if (pq[v] == l) { System.out.println("swimming"); swim(v); }
  ...
  public boolean remove(Key k) {
    return removeByValue(k);
  }
  
  //based on java.util.PriorityQueue.remove(Object)
  public boolean removeByValue(Key k) {
    if (isEmpty()) throw new NoSuchElementException("Priority queue underflow");
    int i = indexOfByValue(k);
    if (i != -1) { removeAtIndex(i); return true; }
    else return false;
  }
  
  //based on java.util.PriorityQueue.removeEq(Object)
  boolean removeByRefEq(Key k) {
    for (int i = 1; i <= size(); i++) {
        if (k == pq[i]) {
          removeAtIndex(i); return true;
        }
    }
    return false;
  }
  ...
  public int indexOf(Key k) {
    return indexOfByValue(k);
  }
  
  // based on java.util.PriorityQueue.indexOf(Object)
  public int indexOfByValue(Key k) {
    if (k != null) {
      for (int i = 1; i < n; i++)
        if (k.equals(pq[i]))
          return i;
  ...
  public int indexOfByRefEq(Key k) {
    if (k != null) {
      for (int i = 1; i < n; i++)
        if (k == (pq[i]))
          return i;
  ...
  public boolean contains(Key k) {
    return indexOfByValue(k) != -1;
  }
  
  //based on java.util.PriorityQueue.contains(Object)
  public boolean containsByValue(Key k) {
    return indexOfByValue(k) != -1;
  }
  
  public boolean containsByRefEq(Key k) {
    return indexOfByRefEq(k) != -1;
  }
...
//    par(toArray());
  ...
  private class HeapIterator implements Iterator<Key> {
    ...
    // create a new pq
    private MaxPQex2429<Key> copy;

    // add all items to copy of heap
    // takes linear time since already in heap order so no keys move
    public HeapIterator() {
      if (comparator == null) copy = new MaxPQex2429<Key>(size());
      else                    copy = new MaxPQex2429<Key>(size(), comparator);
      for (int i = 1; i <= n; i++)
        copy.insert(pq[i]);
    }
  ...
  public static void printHeaps(String[] s) {
    ...
    for (int i = 0; i < hp.length; i++) {
      MaxPQex2429<String> pq = new MaxPQex2429<String>(hq[i]);
      pq.printHeap();
    }     
  }
  ...
  public static void main(String[] args) {
    ...
    Integer[] a = rangeInteger(1,16);
    shuffle(a,r);
    par(a);
    MaxPQex2429<Integer> pq = new MaxPQex2429<>(a);
    pq.show();
    for (int i = 15; i > 1; i--){
      System.out.println(pq.removeAtIndex(i));
      pq.show();
