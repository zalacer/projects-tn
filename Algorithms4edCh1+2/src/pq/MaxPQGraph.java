...
public class MaxPQGraph<Key> implements Iterable<Key> {
   ...
   */
  public MaxPQGraph(int initCapacity) {
   ...
   */
  public MaxPQGraph(int initCapacity, Comparator<Key> comparator) {
   ...
   * @param  comparator the order in which to compare the keys
   */
  public MaxPQGraph(Comparator<Key> comparator) {
    this(1, comparator);
  }
...
//    System.out.println("exch("+pq[i]+","+pq[j]+")");
  ...
  private class HeapIterator implements Iterator<Key> {
    ...
    // create a new pq
    private MaxPQGraph<Key> copy;

    // add all items to copy of heap
    // takes linear time since already in heap order so no keys move
    public HeapIterator() {
      if (comparator == null) copy = new MaxPQGraph<Key>(size());
      else                    copy = new MaxPQGraph<Key>(size(), comparator);
      for (int i = 1; i <= n; i++)
        copy.insert(pq[i]);
    }
  ...
  public static void printHeaps(String[] s) {
    ...
    for (int i = 0; i < hp.length; i++) {
      MaxPQGraph<String> pq = new MaxPQGraph<String>(hq[i]);
      pq.printHeap();
    }     
  }
  ...
  public static Tuple2<Integer,List<Object[]>> findHeapsWithMinOrMaxExchsForXdelMax(
    ...
    MaxPQGraph<Integer>  pq; int min=Integer.MAX_VALUE, max=Integer.MIN_VALUE, e; 
...
//    for (Object[] oa : g) par(oa);
  ...
  MaxPQGraph<Integer> pq;
  pq = new MaxPQGraph<Integer>(9,1,7,2,6,3,5,4,8);
